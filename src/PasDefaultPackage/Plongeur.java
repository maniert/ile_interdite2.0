...
public class Plongeur extends Aventurier {
    ...
    public void setTuileAtteignable(Grille grille) {
        ...
        while (mem != this.getTuileAtteignable()) {
            ...
            while (k <= getTuilessouslocean().size() - 1) {
                System.out.println(tuilesAutour(sec, getTuilessouslocean().get(k), grille).size());
    ...
    public ArrayList<Tuile> tuilesAutour(Etat e, Tuile t, Grille g) {
        ...
        if (existedéjà(g.getTuiles(), g.getLaTuile(x, y - 1))) {
            if (existedéjà(g.getTuiles(), g.getLaTuile(x, y - 1)) && g.getLaTuile(x, y - 1).getEtat() == e) {
        ...
        if (existedéjà(g.getTuiles(), g.getLaTuile(x - 1, y))) {
            if (existedéjà(g.getTuiles(), g.getLaTuile(x - 1, y)) && g.getLaTuile(x - 1, y).getEtat() == e) {
